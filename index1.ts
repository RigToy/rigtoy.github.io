import * as THREE from 'three';

export class MorphSkinCubeApp {
	private scene: THREE.Scene;
	private camera: THREE.PerspectiveCamera;
	private renderer: THREE.WebGLRenderer;
	private mesh: THREE.SkinnedMesh;
	private startTime: number;

	constructor(container: HTMLElement) {
		this.scene = new THREE.Scene();
		this.camera = new THREE.PerspectiveCamera(60, window.innerWidth / window.innerHeight, 0.1, 100);
		this.camera.position.set(0, 1.5, 3);

		this.renderer = new THREE.WebGLRenderer({ antialias: true });
		this.renderer.setSize(window.innerWidth, window.innerHeight);
		container.appendChild(this.renderer.domElement);

		this.startTime = performance.now();

		this.loadTextures().then(textures => {
			this.createSkinnedCube(textures);
			this.animate();
		});

		this.addEvents();
	}

	private async loadTextures(): Promise<THREE.Texture[]> {
		const loader = new THREE.TextureLoader();
		const urls = [
			'/texture.png', '/texture.png',
			'/texture.png', '/texture.png',
			'/texture.png', '/texture.png'
		];
		const textures = await Promise.all(urls.map(url => loader.loadAsync(url)));
		textures.forEach(tex => { tex.flipY = false; });
		return textures;
	}

	private createSkinnedCube(textures: THREE.Texture[]) {
		const size = 1;
		const geo = new THREE.BoxGeometry(size, size, size);
		const count = geo.attributes.position.count;

		// UV per face
		const uv = new Float32Array(count * 2);
		for (let i = 0; i < count; i++) {
			const faceIdx = Math.floor(i / 6);
			uv[i * 2 + 0] = (i % 2 === 0) ? 0.0 : 1.0;
			uv[i * 2 + 1] = (i % 3 === 0) ? 0.0 : 1.0;
		}
		geo.setAttribute('uv', new THREE.BufferAttribute(uv, 2));

		// Morph target
		const posAttr = geo.attributes.position;
		const morph = new Float32Array(count * 3);
		for (let i = 0; i < count; i++) {
			const x = posAttr.getX(i);
			const y = posAttr.getY(i);
			const z = posAttr.getZ(i);
			morph.set([x * 1.2, y * 1.2, z * 1.2], i * 3);
		}
		geo.setAttribute('morphTarget', new THREE.BufferAttribute(morph, 3));

		// Skinning
		const skinIndices = new Uint16Array(count * 4);
		const skinWeights = new Float32Array(count * 4);
		for (let i = 0; i < count; i++) {
			const y = posAttr.getY(i);
			skinIndices[i * 4] = y > 0 ? 1 : 0;
			skinWeights[i * 4] = 1.0;
		}
		geo.setAttribute('skinIndex', new THREE.Uint16BufferAttribute(skinIndices, 4));
		geo.setAttribute('skinWeight', new THREE.Float32BufferAttribute(skinWeights, 4));

		geo.computeVertexNormals();

		// Bones
		const rootBone = new THREE.Bone();
		const childBone = new THREE.Bone();
		rootBone.add(childBone);
		rootBone.position.y = -0.5;
		childBone.position.y = 1.0;

		const skeleton = new THREE.Skeleton([rootBone, childBone]);

		// Shaders
		const vertexShader = `
      precision highp float;
      attribute vec3 position;
      attribute vec3 morphTarget;
      attribute vec2 uv;
      attribute vec4 skinIndex;
      attribute vec4 skinWeight;

      uniform float uMorph;
      uniform mat4 boneMatrices[2];
      uniform mat4 modelViewMatrix;
      uniform mat4 projectionMatrix;
      varying vec2 vUv;

      void main() {
        vec3 morphed = mix(position, morphTarget, uMorph);
        mat4 skinMat =
          skinWeight.x * boneMatrices[int(skinIndex.x)] +
          skinWeight.y * boneMatrices[int(skinIndex.y)];
        vec4 skinned = skinMat * vec4(morphed, 1.0);
        gl_Position = projectionMatrix * modelViewMatrix * skinned;
        vUv = uv;
      }
    `;

		const fragmentShader = `
      precision highp float;
      varying vec2 vUv;
      uniform sampler2D uTexture;
      void main() {
        gl_FragColor = texture2D(uTexture, vUv);
      }
    `;

		const material = new THREE.RawShaderMaterial({
			vertexShader,
			fragmentShader,
			skinning: true,
			uniforms: {
				uMorph: { value: 0.0 },
				uTexture: { value: textures[0] }, // Using one texture for all faces (simplified)
				boneMatrices: { value: skeleton.boneMatrices }
			}
		});

		this.mesh = new THREE.SkinnedMesh(geo, material);
		this.mesh.add(rootBone);
		this.mesh.bind(skeleton);

		this.scene.add(this.mesh);
	}

	private addEvents() {
		window.addEventListener('resize', this.onResize.bind(this));
	}

	private onResize() {
		this.camera.aspect = window.innerWidth / window.innerHeight;
		this.camera.updateProjectionMatrix();
		this.renderer.setSize(window.innerWidth, window.innerHeight);
	}

	private animate = () => {
		requestAnimationFrame(this.animate);
		const time = (performance.now() - this.startTime) / 1000;

		const mat = this.mesh.material as THREE.RawShaderMaterial;
		mat.uniforms.uMorph.value = 0.5 + 0.5 * Math.sin(time * 1.5);

		this.mesh.skeleton.bones[1].rotation.z = Math.sin(time) * 0.5;
		this.mesh.skeleton.update();

		this.renderer.render(this.scene, this.camera);
	};
}

new MorphSkinCubeApp(document.body);
